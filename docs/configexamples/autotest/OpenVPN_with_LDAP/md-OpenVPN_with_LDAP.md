...
![OpenVPN with LDAP topology image](_include/topology.webp)
...
Please look `here <configuration/pki/index:pki>` for more information.

Add the LDAP plugin configuration file <span class="title-ref">/config/auth/ldap-auth.config</span>\
Check all possible settings [here](https://github.com/threerings/openvpn-auth-ldap/blob/master/auth-ldap.conf)

<div class="literalinclude" language="none">

<span id="include">include</span>/ldap-auth.config
...
<span id="include">include</span>/ovpn-server.conf
