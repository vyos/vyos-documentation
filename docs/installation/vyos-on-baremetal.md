...
Refer to `wireless-interface` for additional information, below listed
...
- Intel Corporation AX200 mini-PCIe WiFi module, only supported in mPCIe slot 1.
  (see `wireless-interface-intel-ax200`)

##### WWAN

Refer to `wwan-interface` for additional information, below listed modules
...
the serial port settings (`T1327`).

Create a bootable USB pendrive using e.g. [Rufus](https://rufus.ie/) on a Windows machine.
...
Perform Image installation using <span class="title-ref">install image</span> CLI command.
...
Perform Image installation using <span class="title-ref">install image</span> CLI command. This installation
...
For more information please refer to chapter: `wwan-interface`
