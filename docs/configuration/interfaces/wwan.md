...
`WWAN (Wireless Wide Area Network)` interfaces provide access to cellular
...
**Configure the** `APN (Access Point Name)` **for the WWAN connection.**

Every WWAN connection requires an `APN (Access Point Name)` to connect to
...
factor. The card is installed in a `pc-engines-apu4`.
