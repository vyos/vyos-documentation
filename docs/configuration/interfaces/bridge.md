...
`STP (Spanning Tree Protocol)` is disabled by default in VyOS
and must be explicitly enabled if required. See `stp` for details.
...
Valid interface types are: `ethernet-interface`, `bond-interface`,
`l2tpv3-interface`, `openvpn`, `vxlan-interface`,
`wireless-interface`, `tunnel-interface`, and
`geneve-interface`.
...
**Configure the** `STP (Spanning Tree Protocol)` **port priority
for a specific member interface within a bridge.**

Within the `STP (Spanning Tree Protocol)` topology, each member interface
in a bridge operates as a port with an assigned **priority** and **path cost**.
`STP (Spanning Tree Protocol)` uses these values to determine the
...
`STP (Spanning Tree Protocol)` considers the port priority only if
...
**Configure the** `STP (Spanning Tree Protocol)` **path cost for a
specific member interface within the bridge.**

Path cost is the primary metric `STP (Spanning Tree Protocol)` uses to
...
Configure how bridge interfaces maintain their `FDB (Forwarding Database)`
...
The duration in seconds that a MAC address remains in the bridge’s `FDB 
...
**Configure the** `STP (Spanning Tree Protocol)` **max age timer for
the bridge.**

The duration in seconds that the bridge waits for a `BPDU (Bridge 
Protocol Data Unit)` from the root bridge.

If the bridge does not receive a `BPDU (Bridge Protocol Data Unit)`
...
**Configure the bridge interface to act as the** `IGMP (Internet Group 
Management Protocol)`/`MLD (Multicast Listener Discovery)` **Querier.**

**When configured:** The bridge interface sends `IGMP (Internet Group 
Management Protocol)` (IPv4) and `MLD (Multicast Listener Discovery)`
...
**Configure the bridge interface to perform** `IGMP (Internet Group 
Management Protocol)`/`MLD (Multicast Listener Discovery)`
**snooping.**

**When configured:** The bridge interface monitors `IGMP (Internet Group 
Management Protocol)` (IPv4) and `MLD (Multicast Listener Discovery)`
...
`STP (Spanning Tree Protocol)` is a Layer 2 protocol that prevents loops
...
By default, `STP (Spanning Tree Protocol)` is disabled on bridge interfaces.
...
Enable `STP (Spanning Tree Protocol)` on the bridge interface.
...
**Configure the** `STP (Spanning Tree Protocol)` **delay, in seconds,
...
**Configure the** `STP (Spanning Tree Protocol)` **Hello advertisement
interval, in seconds.**

This parameter sets the frequency at which the bridge interface transmits
Hello packets (`BPDUs (Bridge Protocol Data Units)`). These packets
...
The following example creates a bridge named br100 with `STP (Spanning 
...
Show the `FDB (Forwarding Database)` for the specified bridge.
...
Show the `MDB (Multicast group Database)` for the specified bridge.

The `MDB (Multicast group Database)` is populated by `IGMP 
(Internet Group Management Protocol)`/`MLD (Multicast Listener 
...
show bridge \<name\> macs

Show the learned `MAC (Media Access Control)` address table for the
specified bridge.
