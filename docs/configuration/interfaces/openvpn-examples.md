...
Certificate names are arbitrary. While `openvpn-local` and
...
First, generate a key by running `run generate pki openvpn shared-secret  install <name>` in configuration mode. You can use any name; in this example,
we use `s2s`.
...
For more options, refer to `configuration/pki/index:pki`.
