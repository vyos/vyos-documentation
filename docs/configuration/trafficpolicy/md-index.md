...
[tc](https://en.wikipedia.org/wiki/Tc_(Linux)) is a powerful tool for Traffic Control found at the Linux kernel.
...
In the `creating_a_traffic_policy` section you will see that
...
> </ul>
> </blockquote></td>
> <td>Best effort traffic, default</td>
> </tr>
> <tr>
> <td>001010</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Low</td>
> <td>Assured Forwarding(AF) 11</td>
> </tr>
> <tr>
> <td>001100</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Medium</td>
> <td>Assured Forwarding(AF) 12</td>
> </tr>
> <tr>
> <td>001110</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>High</td>
> <td>Assured Forwarding(AF) 13</td>
> </tr>
> <tr>
> <td>010010</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Low</td>
> <td>Assured Forwarding(AF) 21</td>
> </tr>
> <tr>
> <td>010100</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Medium</td>
> <td>Assured Forwarding(AF) 22</td>
> </tr>
> <tr>
> <td>010110</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>High</td>
> <td>Assured Forwarding(AF) 23</td>
> </tr>
> <tr>
> <td>011010</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Low</td>
> <td>Assured Forwarding(AF) 31</td>
> </tr>
> <tr>
> <td>011100</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Medium</td>
> <td>Assured Forwarding(AF) 32</td>
> </tr>
> <tr>
> <td>011110</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>High</td>
> <td>Assured Forwarding(AF) 33</td>
> </tr>
> <tr>
> <td>100010</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Low</td>
> <td>Assured Forwarding(AF) 41</td>
> </tr>
> <tr>
> <td>100100</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>Medium</td>
> <td>Assured Forwarding(AF) 42</td>
> </tr>
> <tr>
> <td>100110</td>
> <td><blockquote>
...
> </blockquote></td>
> <td>High</td>
> <td>Assured Forwarding(AF) 43</td>
> </tr>
> </tbody>
> </table>
...
As shown in the last command of the example above, the <span class="title-ref">queue-type</span>
...
That is the case of [Shaper](#shaper): each of its classes use fair-queue
...
Some policies can be combined, you will be able to [embed](#embed) a different
...
likely you are looking for a** [Shaper](#shaper) **policy and you want to**
`set its queues <embed>` **as FQ-CoDel**.
...
set qos policy drop-tail \<policy-name\> queue-limit
\<number-of-packets\>
...
[embed](#embed) Fair-Queue into a classful shaping policy to make sure it owns
the queue.
...
FQ-CoDel is based on a modified Deficit Round Robin ([DRR](#DRR)) queue
...
effect. If there is bandwidth available on the physical link, you can
[embed](#embed) FQ-Codel into a classful shaping policy to make sure it owns
...
<span class="title-ref">queue-limit</span> to 1000 packets or less. In rates like 10Mbit, you may
want to set it to 600 packets.

If you are using FQ-CoDel embedded into [Shaper](#shaper) and you have large rates
(100Mbit and above), you may consider increasing <span class="title-ref">quantum</span> to 8000 or
higher so that the scheduler saves CPU.

On low rates (below 40Mbit) you may want to tune <span class="title-ref">quantum</span> down to
something like 300 bytes.

At very low rates (below 3Mbit), besides tuning <span class="title-ref">quantum</span> (300 keeps
being ok) you may also want to increase <span class="title-ref">target</span> to something like 15ms
and increase <span class="title-ref">interval</span> to something around 150 ms.
...
set qos policy fq-codel \<policy-name\> queue-limit
\<number-of-packets\>
...
Limiter is one of those policies that uses [classes](#classes) (Ingress qdisc is
...
**inbound** traffic, check the [ingress-shaping](#ingress-shaping) section.

</div>

<div class="cfgcmd">

set qos policy limiter \<policy-name\> class \<class ID\> match
...
set qos policy limiter \<policy-name\> class \<class-ID\> bandwidth
\<rate\>
...
set qos policy limiter \<policy-name\> class \<class-ID\> burst
\<burst-size\>
...
set qos policy limiter \<policy-name\> class \<class ID\> priority
\<value\>
...
set qos policy network-emulator \<policy-name\> delay
\<delay\>
...
set qos policy network-emulator \<policy-name\> corruption
\<percent\>
...
set qos policy network-emulator \<policy-name\> loss
\<percent\>
...
set traffic-policy network-emulator \<policy-name\> reordering
\<percent\>
...
set traffic-policy network-emulator \<policy-name\> queue-limit
\<limit\>
...
bandwidth available on the physical link, you can [embed](#embed) Priority
Queue into a classful shaping policy to make sure it owns the queue.
In that case packets can be prioritized based on DSCP.

</div>

Up to seven queues -defined as [classes](#classes) with different priorities- can
...
In Priority Queue we do not define clases with a meaningless
...
As with other policies, you can [embed](#embed) other policies into the classes
...
IP precedence as defined in `791`:
...
set qos policy random-detect \<policy-name\> bandwidth \<bandwidth\>
...
set qos policy random-detect \<policy-name\> precedence
...
different [classes](#classes) you can configure (up to 4096). You can [embed](#embed) a
...
set qos policy round-robin \<policy name\> class
...
As with other policies, Round-Robin can [embed](#embed) another policy into a
...
set qos policy shaper \<policy-name\> class \<class-ID\> bandwidth
\<rate\>
...
set qos policy shaper \<policy-name\> class \<class-ID\> burst
\<bytes\>
...
set qos policy shaper \<policy-name\> class \<class-ID\> ceiling
\<bandwidth\>
...
set qos policy shaper \<policy-name\> class \<class-ID\> priority
\<0-7\>
...
As with other policies, Shaper can [embed](#embed) other policies into its
...
[Common Applications Kept Enhanced](https://www.bufferbloat.net/projects/codel/wiki/Cake/) (CAKE) is a comprehensive queue management
...
([Intermediate Functional Block](https://www.linuxfoundation.org/collaborate/workgroups/networking/ifb)). There, in that virtual interface,
