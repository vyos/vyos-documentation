...
VyOS utilizes [accel-ppp]() to provide PPPoE server functionality. It can
...
set service pppoe-server authentication local-users username
\<name\> password \<password\>

Create <span class="title-ref">\<user\></span> for local authentication on this system. The users password
will be set to <span class="title-ref">\<pass\></span>.

</div>

<div class="cfgcmd">

set service pppoe-server client-ip-pool \<POOL-NAME\>
...
Specifies single <span class="title-ref">\<gateway\></span> IP address to be used as local address of PPP
interfaces.
...
set service pppoe-server authentication radius dynamic-author
key \<secret\>
...
add new [dictionary](https://github.com/accel-ppp/accel-ppp/blob/master/accel-pppd/radius/dict/dictionary.rfc6911).
...
It is custom [ACCEL-PPP attribute](https://github.com/accel-ppp/accel-ppp/blob/master/accel-pppd/radius/dict/dictionary.accel). Define it in your RADIUS server.
...
Download bandwidth limit in kbit/s for <span class="title-ref">\<user\></span>.
...
Upload bandwidth limit in kbit/s for <span class="title-ref">\<user\></span>.
...
set service pppoe-server pado-delay \<number-of-ms\>
...
set service pppoe-server ppp-options ipv6-interface-id
\<random | x:x:x:x\>
...
set service pppoe-server extended-scripts on-change \<path_to_script\>
...
set service pppoe-server extended-scripts on-down \<path_to_script\>
...
set service pppoe-server extended-scripts on-pre-up \<path_to_script\>
...
set service pppoe-server extended-scripts on-up \<path_to_script\>
...
Disable <span class="title-ref">\<user\></span> account.
...
Assign static IP address to <span class="title-ref">\<user\></span> account.

</div>

<div class="cfgcmd">

set service pppoe-server authentication protocols
...
set service pppoe-server ppp-options interface-cache \<number\>
...
set service pppoe-server ppp-options ipv4
...
set service pppoe-server ppp-options lcp-echo-failure \<number\>

Defines the maximum <span class="title-ref">\<number\></span> of unanswered echo requests. Upon reaching the
value <span class="title-ref">\<number\></span>, the session will be reset. Default value is **3**.

</div>

<div class="cfgcmd">

set service pppoe-server ppp-options lcp-echo-interval \<interval\>

If this option is specified and is greater than 0, then the PPP module will
send LCP pings of the echo request every <span class="title-ref">\<interval\></span> seconds.
Default value is **30**.
...
set service pppoe-server ppp-options lcp-echo-timeout
...
Specifies `MPPE (Microsoft Point-to-Point Encryption)` negotiation
preference.
...
set service pppoe-server description \<description\>
...
set service pppoe-server limits burst \<value\>
...
set service pppoe-server limits connection-limit \<value\>
...
set service pppoe-server limits timeout \<value\>
...
set service pppoe-server max-concurrent-sessions
...
set service pppoe-server service-name \<names\>
...
set multiple service-names: <span class="title-ref">sn1,sn2,sn3</span>
...
set service pppoe-server session-control
...
set service pppoe-server shaper fwmark \<1-2147483647\>
...
set service pppoe-server wins-server \<address\>
