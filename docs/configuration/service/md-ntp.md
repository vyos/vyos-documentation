...
milliseconds of `UTC (Coordinated Universal Time)`. It uses the
...
documented in `5905`. It is backward compatible with version 3, specified
in `1305`.
...
VyOS 1.4 uses chrony instead of ntpd (see `T3008`) which will
...
the <span class="title-ref">allow-client</span> directive.
...
an IP address or `FQDN (Fully Qualified Domain Name)`.
...
the <span class="title-ref">\<address\></span> and optionally the permitted clients. Multiple listen
...
set service ntp leap-second \[ignore[|smear|](##SUBST##|smear|)system|timezone\]
