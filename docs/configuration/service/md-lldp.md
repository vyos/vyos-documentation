...
`NDP (Nortel Discovery Protocol)` and `LLTD (Link Layer Topology
Discovery)`.

Information gathered with LLDP is stored in the device as a `MIB
(Management Information Database)` and can be queried with `SNMP (Simple
Network Management Protocol)` as specified in `2922`. The topology of an
...
Enable transmission of LLDP information on given <span class="title-ref">\<interface\></span>. You can also
...
Disable transmit of LLDP frames on given <span class="title-ref">\<interface\></span>. Useful to exclude
...
set service lldp legacy-protocols \<cdp[|edp|](##SUBST##|edp|)fdp|sonmp\>
...
Show LLDP neighbors connected via interface <span class="title-ref">\<interface\></span>.
