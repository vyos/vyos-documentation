...
set system option kernel amd-pstate-driver \<mode\>
...
<span class="title-ref">source-address</span> and <span class="title-ref">source-interface</span> can not be used at the same
time.
...
set system option ssh-client source-address \<address\>
...
set system option ssh-client source-interface \<interface\>
...
As more and more routers run on Hypervisors, especially with a `NOS
...
We now utilize <span class="title-ref">tuned</span> for dynamic resource balancing based on profiles.
