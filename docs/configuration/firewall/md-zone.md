...
found in the [firewall](https://docs.vyos.io/en/latest/configuration/firewall/general.html)
chapter. The legacy firewall is still available for versions before
1.4-rolling-202308040557 and can be found in the
`legacy firewall configuration </configuration/firewall/general-legacy>`
chapter.
...
The zone configuration moved from `zone-policy zone <name>` to `firewall zone <name>`.
...
A basic introduction to zone-based firewalls can be found [here](https://support.vyos.io/en/kb/articles/a-primer-to-zone-based-firewall),
and an example at `examples-zone-policy`.
...
set firewall zone \<Destination Zone\> from \<Source Zone\>
...
set firewall zone \<name\> from \<name\> firewall name
\<rule-set\>
...
set firewall zone \<name\> from \<name\> firewall ipv6-name
\<rule-set\>
