...
**firewall global-options all-ping** affects only to LOCAL
...
set firewall global-options source-validation
...
Enable or Disable VyOS to be `1337` conform.
The following system parameter will be altered:
...
set firewall global-options state-policy related log-level
\[emerg | alert | crit | err | warn | notice | info | debug\]
