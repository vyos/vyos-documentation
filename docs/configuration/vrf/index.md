...
`VRF (Virtual Routing and Forwarding)` devices combined with ip rules
...
Create a new VRF instance with <span class="title-ref">\<name\></span> and <span class="title-ref">\<id\></span>. The name is used when placing
...
Assign interface identified by <span class="title-ref">\<interface\></span> to VRF named <span class="title-ref">\<name\></span>.
...
difference is, that each routing protocol used, must be prefixed with the <span class="title-ref">vrf
name \<name\></span> command.
...
- `routing-bgp`: `set vrf name <name> protocols bgp ...`
- `routing-isis`: `set vrf name <name> protocols isis ...`
- `routing-ospf`: `set vrf name <name> protocols ospf ...`
- `routing-ospfv3`: `set vrf name <name> protocols ospfv3 ...`
- `routing-static`: `set vrf name <name> protocols static ...`
...
Display IPv4 routing table for VRF identified by <span class="title-ref">\<name\></span>.
...
Display IPv6 routing table for VRF identified by <span class="title-ref">\<name\></span>.
...
identified by <span class="title-ref">\<name\></span>. When using the IPv4 or IPv6 option, displays the
...
`L3VPN VRFs ( Layer 3 Virtual Private Networks )` bgpd supports for
...
\<ipv4-unicast[|ipv6-unicast\> rd vpn export \<asn:nn|](##SUBST##|ipv6-unicast> rd vpn export <asn:nn|)address:nn\>
...
set vrf name \<name\> protocols bgp address-family
\<ipv4-unicast[|ipv6-unicast\> route-target vpn \<import|](##SUBST##|ipv6-unicast> route-target vpn <import|)export|both\>
\[RTLIST\]
...
set vrf name \<name\> protocols bgp address-family
\<ipv4-unicast[|ipv6-unicast\> label vpn export \<0-1048575|](##SUBST##|ipv6-unicast> label vpn export <0-1048575|)auto\>
...
set vrf name \<name\> protocols bgp address-family
\<ipv4-unicast[|ipv6-unicast\> route-map vpn \<import|](##SUBST##|ipv6-unicast> route-map vpn <import|)export\>
\[route-map \<name\>\]
...
set vrf name \<name\> protocols bgp address-family
\<ipv4-unicast[|ipv6-unicast\> \<import|](##SUBST##|ipv6-unicast> <import|)export\> vpn
...
set vrf name \<name\> protocols bgp interface \<interface\> mpls
forwarding
