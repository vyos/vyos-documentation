...
`SSTP (Secure Socket Tunneling Protocol)` is a form of `VPN
...
VyOS utilizes [accel-ppp]() to provide SSTP server functionality. We support both
...
set vpn sstp authentication local-users username \<user\> password
\<pass\>
...
purposes. This is a custom [ACCEL-PPP attribute](https://github.com/accel-ppp/accel-ppp/blob/master/accel-pppd/radius/dict/dictionary.accel). Define it in your RADIUS
server.
...
set vpn sstp client-ipv6-pool \<IPv6-POOL-NAME\> prefix \<address\>
mask \<number-of-bits\>
...
set vpn sstp client-ipv6-pool \<IPv6-POOL-NAME\> delegate \<address\>
...
set vpn sstp authentication local-users username \<user\> static-ip
\<address\>

Assign a static IP address to <span class="title-ref">\<user\></span> account.

</div>

<div class="cfgcmd">

set vpn sstp authentication local-users username \<user\> rate-limit
download \<bandwidth\>

Rate limit the download bandwidth for <span class="title-ref">\<user\></span> to <span class="title-ref">\<bandwidth\></span> kbit/s.
...
Rate limit the upload bandwidth for <span class="title-ref">\<user\></span> to <span class="title-ref">\<bandwidth\></span> kbit/s.

</div>

<div class="cfgcmd">

set vpn sstp authentication protocols
...
send LCP echo requests every <span class="title-ref">\<interval\></span> seconds.
Default value is **30**.
...
Connected clients should use <span class="title-ref">\<address\></span> as their DNS server. This command
...
testing. The Linux client used for testing is called [sstpc](https://github.com/reliablehosting/sstp-client). [sstpc](https://github.com/reliablehosting/sstp-client) requires a
