...
VyOS utilizes [accel-ppp](https://accel-ppp.org/) to provide L2TP server functionality. It can be used
...
set vpn l2tp remote-access authentication local-users username \<user\> password
\<pass\>
...
Some [RADIUS](https://en.wikipedia.org/wiki/RADIUS) severs use an access control list which allows or denies
...
To explain the usage of LNS follow our blueprint `examples-lac-lns`.
...
set vpn l2tp remote-access authentication local-users username \<user\> static-ip
\<address\>
...
set vpn l2tp remote-access authentication local-users username \<user\> rate-limit
download \<bandwidth\>
...
set vpn l2tp remote-access authentication protocols
