...
The Point-to-Point Tunneling Protocol ([PPTP](#pptp)) has been implemented in VyOS only
...
set vpn pptp remote-access authentication local-users username \<user\> password
\<pass\>
...
set vpn pptp remote-access authentication local-users username \<user\> static-ip
\<address\>
...
set vpn pptp remote-access authentication local-users username \<user\> rate-limit
download \<bandwidth\>
...
set vpn pptp remote-access authentication protocols
