...
`DMVPN (Dynamic Multipoint Virtual Private Network)` is a dynamic
`VPN (Virtual Private Network)` technology originally developed by Cisco.
...
- `NHRP (Next Hop Resolution Protocol)` `2332`
- `mGRE (Multipoint Generic Routing Encapsulation)` `1702`
- `IPSec (IP Security)` - too many RFCs to list, but start with
  `4301`
...
- Please refer to the `tunnel-interface` documentation for the individual
  tunnel related options.
- Please refer to the `ipsec_general` documentation for individual IPSec
  related options.
...
set protocols nhrp tunnel \<tunnel\> dynamic-map \<address\>
nbma-domain-name \<fqdn\>

Specifies that the `NBMA (Non-broadcast multiple-access network)`
...
It is very easy to misconfigure multicast repeating if you have
multiple NHSes.
...
multiple spoke sites. The lab was built using `EVE-NG (Emulated Virtual
Environment NG)`.
...
The below referenced IP address <span class="title-ref">192.0.2.1</span> is used as example address
