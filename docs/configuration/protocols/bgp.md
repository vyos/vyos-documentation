...
Peer groups improve scalability by allowing the same update information
to be generated once for all group members. Routes generated by a peer
group member are sent back to the originating peer with the originator
identifier attribute identifying the source. Peers not assigned to a peer
group are treated as members of the default peer group and share updates.
...
```{note}
By default, BGP advertises the configured prefix even when it is not
present in the routing table. Other vendor implementations require the
prefix to be in the RIB before advertising it.
```
