...
- ``/var/log/vpp.log`` - This files contains logs related to daemon startup and log of commands executed directly via VPP CLI. Pay attention, VyOS does not use VPP CLI for the configuration, so this log will not contain any configuration changes made via VyOS CLI, and should not be informative in most cases.
