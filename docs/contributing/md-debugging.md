...
you can follow the steps at `iso_build_issues`.
...
- `vyos-debug` - Adding the parameter to the linux boot line will produce
  ...
  in [vyatta-cfg](https://github.com/vyos/vyatta-cfg). Output is directed to `/var/log/vyatta/cfg-stdout.log`.
...
using this file: [bootchart.conf](https://github.com/vyos/vyos-build/blob/current/data/live-build-config/includes.chroot/etc/systemd/bootchart.conf)
